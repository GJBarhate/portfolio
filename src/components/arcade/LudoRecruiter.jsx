...
export default function LudoRecruiter() {
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full max-w-sm mx-auto">
      <div className="flex items-center gap-3 font-mono text-[10px] tracking-wider text-[var(--ink-faint)]">
      ...
      <div className="w-full p-3 rounded-xl clay--inset space-y-2">
        {TOKENS.map((token, i) => (
          <div key={token.id} className="relative flex items-center gap-2">
            ...
            <div className="flex-1 h-5 rounded-full overflow-hidden" style={{ background: 'var(--void-3)' }}>
              <motion.div
            ...
            <span className="w-6 text-right font-mono text-[9px] text-[var(--ink-faint)]">{positions[i]}/{token.pathLen}</span>
          </div>
        ))}
      </div>
      ...
      <div className="w-full max-w-xs">
        <p className="font-mono text-[9px] tracking-wider text-[var(--ink-faint)] mb-1">RACE LOG</p>
        <div className="space-y-0.5 max-h-24 overflow-y-auto">
          {log.map((entry, i) => (
            <p key={i} className="text-[9px] font-mono text-[var(--ink-dim)]">{entry}</p>
          ))}
        </div>
      </div>
    </div>
